...
<html>
  <head>
    <title>Luitjens Helm Charts</title>
  ...
  <code id="helm-search-command">helm search repo luitjens-test-gh-app --versions</code>
      ...
      <p>
        <ul>
          <li><a href="https://github.com/luitjens-test-gh-app/helm-charts">Helm-charts GitHub</a></li>
        </ul>
      </p>
        ...
        {{ if not (index $chartEntry 0).Deprecated }}
          <div class="chart">
            <a href="{{ (index (index $chartEntry 0).Urls 0) }}" title="{{ (index (index $chartEntry 0).Urls 0) }}">
              <div class="icon">
                <img class="chart-item-logo" alt="{{ $key }}'s logo" src="_images/{{ (index $chartEntry 0).Name }}.png" onerror="this.onerror=null; this.src='_images/placeholder.png';">
              </div>
              <div class="body">
                <p class="info">
